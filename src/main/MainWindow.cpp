...
void MainWindow::createActionsFile ()
{
  ...
  m_actionImportSortedPoints = new QAction (tr ("Import (Sorted Points)..."), this);
  m_actionImportSortedPoints->setStatusTip (tr ("Imports a set of sorted points into the current document."));
  m_actionImportSortedPoints->setWhatsThis (tr ("Import (Sorted Points)\n\n"
                                                "Imports sorted points from a file one-by-one into the current document. The Undo command can "
                                                "be used afterward to remove points one-by-one that were imported. The sorted points are "
                                                "typically generated by other software tools, such as python scripts. The points are typically "
                                                "ordered from best to worst so applying Undo removes points that are not as good."));
  connect (m_actionImportSortedPoints, SIGNAL (triggered ()), this, SLOT (slotFileImportSortedPoints ()));
...
void MainWindow::createMenus()
{
  ...
  m_menuFile->addAction (m_actionImportSortedPoints);
...
void MainWindow::slotDigitizeAxis ()
{
  ...
  updateControls (); // Change state-dependent controls
}
...
void MainWindow::slotFileImportSortedPoints ()
{
  LOG4CPP_INFO_S ((*mainCat)) << "MainWindow::slotFileImportSortedPoints";

  m_digitizeStateContext->handleFileImportSortedPoints (m_cmdMediator);
}
...
void MainWindow::updateControls ()
{
  ...
  m_actionImportSortedPoints->setEnabled (m_cmdMediator != 0 && m_digitizeStateContext->isEnableImportSortedPoints ());
#ifndef OSX_RELEASE
